...
def generate_agents(diameter, boundaryControl, initial_speed, sensorDist, sensorSize, sensorAngle, maxTurnAngle, agent_number):
    locations = [generate_sample(diameter, radius=int(
        diameter/2 - boundaryControl)) for i in range(agent_number)]
...
def generate_Food(foodNumber, diameter, boundaryControl, foodWeight, mask, random_food=False, foodLocation=None):
    if random_food:
        ...
        foodLocation = [generate_sample(diameter, radius=int(
            diameter/2-2*boundaryControl)) for i in range(foodNumber)]
    else:
        foodLocation = foodLocation

    foodlayer = getGaussianMap(mapSize=diameter, diffusionVariance=[
        200], foodLocations=foodLocation, meanValue=foodWeight, mask=mask)
...
@cuda.jit
def one_step_simpulation(agent_location, petridish, angle, maxTurnAngle, distance, occupied, sensorAngle, rng_states):
    start = cuda.grid(1)
    ...
    for agent_id in range(start, agent_location.shape[0], stride):
        ...
        if occupied[next_location_x, next_location_y] == 0:
            ...
            angle[agent_id] = xoroshiro128p_uniform_float32(rng_states, agent_id) * \
                math.pi*2
...
def update_occupied(agent_location, occupied):
    ...
    for location in range(start, agent_location.shape[0], stride):
        occupied[agent_location[location][0],
                 agent_location[location][1]] = 255
...
def update_petridish(agent_location, petridish):
    ...
    stride = cuda.gridsize(1)
    for location in range(start, agent_location.shape[0], stride):
        petridish[agent_location[location][0],
                  agent_location[location][1]] += 0.5
...
if __name__ == "__main__":
    ...
    slimes = generate_agents(diameter, boundaryControl, initial_speed,
                             sensorDist, sensorSize, sensorAngle, maxTurnAngle, agent_number)
    ...
    foodlayer = generate_Food(
        foodNumber, diameter, boundaryControl, 5, mask, random_food=False, foodLocation=foodLocation)
    ...
    iterations = 10000
    while s < iterations:
        # Using streams for kernel launches and memory copies
        set_zeros[blockspergrid, threadsperblock, stream1](occupied_device)
        update_occupied[1024, 1024, stream1](locations_device, occupied_device)

        if s % 50 == 0:
            print(f"******This is {s} of {iterations}*******")
            # Asynchronous copy to host after sync
            stream1.synchronize()
            occupied = occupied_device.copy_to_host(stream=stream2)
            occupied_frame.append(draw_occupied(occupied, foodlayer))

        one_step_simpulation[1024, 1024, stream1](
            locations_device, petridish_device, angles_device, maxTurnAngle, sensorDist, occupied_device, sensorAngle, rng_states)

        update_petridish[1024, 1024, stream1](locations_device, petridish_device)
        evaporate[blockspergrid, threadsperblock, stream1](petridish_device)
        s += 1

    elapsed = timer() - start
    print(f'Main loop time: {elapsed}')
