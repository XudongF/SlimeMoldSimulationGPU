...
def plot_graph(G, node_dict):
    ...
    nx.draw_networkx(G, flipped_pos, node_color='red', nodelist=plot_nodes,
                     edge_color='green', arrows=False, with_labels=False, node_size=5, ax=ax)
...
def validate_number(nodes, buffer, threshold):
    for k in range(1, len(nodes)+1):
        if k > threshold:
            ...
            max_distance = np.max([math.dist(centers[idx], node) for idx in range(k) for node in list(
                compress(nodes, labels == idx))])
    ...
    for idx in range(len(centers)):
        ...
        for node in list(compress(nodes_location, labels == idx)):
            ...
            G = nx.contracted_nodes(
                G, new_id, node_id, copy=False, self_loops=False)
    return G
...
def align_to_original(G, original_nodes):
    ...
    for node, pos in original_nodes.items():
        ...
        if min(node_dist) < 100:
            ...
            G = nx.contracted_nodes(
                G, node, near_node, copy=False, self_loops=False)
...
def align_to_origina_test(G, original_nodes):
    ...
    for node, pos in original_nodes.items():
        ...
        if near_node != None:
            ...
            for u, v in G.edges():
                if (angle_between(G.nodes[u]['o'] - G.nodes[v]['o'], true_pos - G.nodes[v]['o']) < math.pi/2) and (angle_between(G.nodes[v]['o'] - G.nodes[u]['o'], true_pos - G.nodes[u]['o']) < math.pi/2):
                    dist_edge = np.linalg.norm(np.abs(np.cross(
                        G.nodes[v]['o']-G.nodes[u]['o'], G.nodes[u]['o'] - true_pos)))/np.linalg.norm(G.nodes[v]['o']-G.nodes[u]['o'])
                    if dist_edge < tt:
                        tt = dist_edge
                        edge_1, edge_2 = u, v
                else:
            ...
            if edge_1 is not None:
                ...
                plot_graph(G, node_dict={
                           node: original_nodes[node], edge_1: G.nodes[edge_1]['o'], edge_2: G.nodes[edge_1]['o']})
...
def draw_process(time_step, node_dict, binary=None, skele=None, simplified=None, save_path=None):
    ...
    if skele is not None:
        ...
        plt.savefig(os.path.join(
            save_path, f'Skeleted{time_step}.jpg'), dpi=300)
        plt.show()
    if simplified is not None:
        fig, ax = plt.subplots()
        plot_graph(simplified, node_dict)
        plt.savefig(os.path.join(
            save_path, f'Simplified{time_step}.jpg'), dpi=300)
        plt.show()


def generate_network(img_data, buffer, diameter, boundaryControl, threshold=200, node_dict=None):
    ...
    G_consolidated = consolidate_graph(
        G_ske, buffer=buffer, threshold=threshold)
...
def getGaussianMap(mapSize, diffusionVariance, foodLocations, meanValue, mask):
    ...
    for foodID in np.arange(len(foodLocations)):
        ...
        if len(diffusionVariance) > 1:
            diffusion = np.array([[diffusionVariance[foodID], 0], [
                                 0, diffusionVariance[foodID]]])

        else:
            diffusion = np.array(
                [[diffusionVariance[0], 0], [0, diffusionVariance[0]]])

        Z += multivariate_gaussian(pos,
                                   np.array(foodLocations[foodID]), diffusion)
...
def get_network(file, boundaryControl):
    ...
    # # %%
    node_location = [np.rint(np.array((row.X, row.Y), dtype=np.float32))
    ...
    return diameter, node_dict, netfile,
